...
func SetCompact(nCompact uint32) (res *big.Int) {
	size := nCompact>>24
	//fmt.Println("ncompact size:", size)
	...
	//fmt.Println("setCompact result", res)
	return res
}
...
func GetCompact(b *big.Int) uint32 {
	...
	fmt.Println("GetCompact", "difficulty", b, "size", size)
	if size <= 3 {
...
func ChecReservekPOW(hash []byte, bits uint32) bool {
	//return HashToBig(hash).Cmp(SetCompact(bits)) <= 0
	buf := make([]byte, 32)
	copy(buf, hash)
	blen := len(buf)
	for i := 0; i < blen/2; i++ {
	...
	return new(big.Int).SetBytes(hash).Cmp(SetCompact(bits)) <= 0
}
...
func DiffToTarget(diff *big.Int) *big.Int {
	...
	//fmt.Println("maxTarget ", maxTarget, "diff", diff)
	target := new(big.Int).Div(maxTarget, diff)
	//fmt.Println("Diff to Target", "Diff", diff, "target", target)
	return target
}
