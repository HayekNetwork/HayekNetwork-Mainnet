...
import (
	...
	"github.com/hayekchain/go-hayekchain/internal/testlog"
...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom hykash engine.
	hykash := NewTester([]string{server.URL}, false)
	defer hykash.Close()
	...
	hykash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := hykash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom hykash engine.
	hykash := NewTester([]string{server.URL}, false)
	hykash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer hykash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			hykash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, hykash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch hykash result timeout", id+1)
		}
	}
}
