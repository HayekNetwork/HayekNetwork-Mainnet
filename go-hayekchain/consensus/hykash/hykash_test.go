...
// Tests that hykash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	hykash := NewTester(nil, false)
	defer hykash.Close()

	results := make(chan *types.Block)
	err := hykash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := hykash.VerifySeal(nil, header); err != nil {
...
// It reproduces https://github.com/hayekchain/go-hayekchain/issues/14943
func TestCacheFileEvict(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "hykash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func verifyTest(wg *sync.WaitGroup, e *Hayekash, workerIndex, epochs int) {
	defer wg.Done()
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := hykash.SealHash(header)

	// Push new work.
	results := make(chan *types.Block)
	hykash.Seal(nil, block, results, nil)
...
func TestHashRate(t *testing.T) {
	...
	if tot := hykash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	api := &API{hykash}
	if _, err := api.GetWork(); err != errHayekashStopped {
		t.Error("expect to return an error to indicate hykash is stopped")
	}

	if res := api.SubmitHashRate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped hykash")
	}
}
