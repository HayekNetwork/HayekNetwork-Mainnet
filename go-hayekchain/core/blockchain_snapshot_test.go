...
// (i)   Ghyk restarts normally with valid legacy snapshot
// (ii)  Ghyk restarts normally with valid new-format snapshot
// (iii) Ghyk restarts after the crash, with broken legacy snapshot
// (iv)  Ghyk restarts after the crash, with broken new-format snapshot
// (v)   Ghyk restarts normally, but it's requested to be rewound to a lower point via SetHead
// (vi)  Ghyk restarts normally with a stale snapshot
type snapshotTest struct {
	legacy       bool   // Flag whether the loaded snapshot is in legacy format
	crash        bool   // Flag whether the Ghyk restarts from the previous crash
...
// Tests a Ghyk restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a Ghyk restart with valid but "legacy" snapshot. Before the shutdown,
...
// Tests a Ghyk was crashed and restarts with a broken snapshot. In this case the
...
// Tests a Ghyk was crashed and restarts with a broken snapshot. In this case
...
// Tests a Ghyk was crashed and restarts with a broken and "legacy format"
...
// Tests a Ghyk was running with snapshot enabled. Then restarts without
...
// Tests a Ghyk was running with leagcy snapshot enabled. Then restarts
...
// Tests the Ghyk was running with snapshot enabled and resetHead is applied.
...
// Tests the Ghyk was running with snapshot(legacy-format) enabled and resetHead
...
// Tests the Ghyk was running with snapshot(legacy-format) enabled and upgrades
// the disk layer journal(journal generator) to latest format. After that the Ghyk
// is restarted from a crash. In this case Ghyk will find the new-format disk layer
...
func testSnapshot(t *testing.T, tt *snapshotTest) {
	...
	chain, err := NewBlockChain(db, cacheConfig, params.AllHayekashProtocolChanges, engine, vm.Config{}, nil, nil)
	if err != nil {
	...
	if tt.crash {
		...
		chain, err = NewBlockChain(db, nil, params.AllHayekashProtocolChanges, engine, vm.Config{}, nil, nil)
		if err != nil {
