...
func wipeSnapshot(db hykdb.KeyValueStore, full bool) chan struct{} {
...
func wipeContent(db hykdb.KeyValueStore) error {
...
func wipeKeyRange(db hykdb.KeyValueStore, kind string, prefix []byte, keylen int) error {
	// Batch deletions toghyker to avoid holding an iterator for too long
	var (
		batch = db.NewBatch()
		items int
	)
