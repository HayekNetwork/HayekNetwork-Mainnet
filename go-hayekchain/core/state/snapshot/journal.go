...
type journalGenerator struct {
	Wiping   bool // Whhyker the database was in progress of being wiped
	Done     bool // Whhyker the generator finished creating the snapshot
	Marker   []byte
	Accounts uint64
	Slots    uint64
	Storage  uint64
...
func loadAndParseLegacyJournal(db hykdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
...
func loadAndParseJournal(db hykdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Ghyk
	...
	// It can happen that Ghyk crashes without persisting the latest
	// diff journal.
...
func loadSnapshot(diskdb hykdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, error) {
	...
	//
	// Possible scenario: Ghyk was crashed without persisting journal and then
	...
	if !generator.Done {
		...
		// Whhyker or not wiping was in progress, load any generator progress too
