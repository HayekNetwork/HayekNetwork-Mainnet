...
// Package core implements the HayekChain consensus protocol.
package core
...
import (
	...
	"github.com/hayekchain/go-hayekchain/core/state/snapshot"
...
type CacheConfig struct {
	...
	TrieCleanNoPrefetch bool          // Whhyker to disable heuristic state prefetching for followup blocks
	TrieDirtyLimit      int           // Memory limit (MB) at which to start flushing dirty trie nodes to disk
	TrieDirtyDisabled   bool          // Whhyker to disable trie write caching and GC altoghyker (archive node)
	TrieTimeLimit       time.Duration // Time limit after which to flush the current in-memory trie to disk
	SnapshotLimit       int           // Memory allowance (MB) to use for caching snapshot entries in memory
	Preimages           bool          // Whhyker to store preimage of trie key to the disk
...
type BlockChain struct {
	...
	db     hykdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default HayekChain Validator and
// Processor.
func NewBlockChain(db hykdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// the head block (hykash cache or clique voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) SetHeadBeyondRoot(head uint64, root common.Hash) (uint64, error) {
	...
	updateFn := func(db hykdb.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db hykdb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	// touching the header chain altoghyker, unless the freezer is broken
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from Ghyk
			...
			//
			// But if the `TxIndexTail` is not nil, e.g. Ghyk is initialized with
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// tx indexes)
			if batch.ValueSize() >= hykdb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - hykdb.IdealBatchSize)
			}
...
// sync, Ghyk will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Ghyk with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Ghyk to a new version which supports transaction
