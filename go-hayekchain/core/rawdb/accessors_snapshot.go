...
func ReadSnapshotRoot(db hykdb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db hykdb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db hykdb.KeyValueWriter) {
...
func ReadAccountSnapshot(db hykdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db hykdb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db hykdb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db hykdb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db hykdb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db hykdb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db hykdb.Iteratee, accountHash common.Hash) hykdb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db hykdb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db hykdb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db hykdb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db hykdb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db hykdb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db hykdb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db hykdb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db hykdb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db hykdb.KeyValueWriter) {
