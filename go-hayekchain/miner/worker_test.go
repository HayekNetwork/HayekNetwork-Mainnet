...
var (
	...
	hykashChainConfig *params.ChainConfig
...
func init() {
	...
	hykashChainConfig = params.TestChainConfig
...
func newTestWorkerBackend(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db hykdb.Database, n int) *testWorkerBackend {
...
func newTestWorker(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db hykdb.Database, blocks int) (*worker, *testWorkerBackend) {
	backend := newTestWorkerBackend(t, chainConfig, engine, db, blocks)
	backend.txPool.AddLocals(pendingTxs)
	w := newWorker(testConfig, chainConfig, engine, backend, new(event.TypeMux), nil, false)
	w.setHayekerbase(testBankAddress)
	return w, backend
}

func TestGenerateBlockAndImportHayekash(t *testing.T) {
...
func TestEmptyWorkHayekash(t *testing.T) {
	testEmptyWork(t, hykashChainConfig, hykash.NewFaker())
}
...
func TestStreamUncleBlock(t *testing.T) {
	hykash := hykash.NewFaker()
	defer hykash.Close()

	w, b := newTestWorker(t, hykashChainConfig, hykash, rawdb.NewMemoryDatabase(), 1)
	defer w.close()
...
func TestRegenerateMiningBlockHayekash(t *testing.T) {
	testRegenerateMiningBlock(t, hykashChainConfig, hykash.NewFaker())
}
...
func TestAdjustIntervalHayekash(t *testing.T) {
	testAdjustInterval(t, hykashChainConfig, hykash.NewFaker())
}
