...
type request struct {
	...
	code bool        // Whhyker this is a code entry
...
type Sync struct {
	database hykdb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database hykdb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them toghyker
...
// both hyk/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw hykdb.Batch) error {
