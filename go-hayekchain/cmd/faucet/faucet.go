...
// faucet is a Hayeker faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the HayekChain protocol")
	statsFlag   = flag.String("hykstats", "", "Hykstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Hayekers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for HayekChain and the faucet")
)

var (
	hyker = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Hayekers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // HayekChain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an HayekChain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // HayekChain protocol stack
	client *hykclient.Client   // Client connection to the HayekChain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the HayekChain light client protocol
	cfg := hyk.DefaultConfig
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the HayekChain service: %w", err)
	}

	// Assemble the hykstats monitoring and reporting service'
	if stats != "" {
		if err := hykstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := hykclient.NewClient(api)
...
// close terminates the HayekChain connection and tears down the faucet.
...
// apiHandler handles requests for Hayeker grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, hyker),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the HayekChain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/hayekchain/go-hayekchain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), hyker)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, hyker)
			peers := f.stack.Server().PeerCount()
...
// the username, avatar URL and HayekChain address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// and scrape it for the HayekChain address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No HayekChain address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and HayekChain address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the HayekChain address and profile URL.
	res, err := http.Get(url)
	if err != nil {
		return "", "", common.Address{}, err
	}
...
// authNoAuth tries to interpret a faucet request as a plain HayekChain address,
