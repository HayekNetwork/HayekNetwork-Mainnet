...
// ghyk is the official command-line client for HayekChain.
package main
...
import (
	...
	"github.com/hayekchain/go-hayekchain/console/prompt"
...
const (
	clientIdentifier = "ghyk" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-hayekchain command line interface")
	...
	nodeFlags = []cli.Flag{
		...
		utils.HayekashCacheDirFlag,
		utils.HayekashCachesInMemoryFlag,
		utils.HayekashCachesOnDiskFlag,
		utils.HayekashCachesLockMmapFlag,
		utils.HayekashDatasetDirFlag,
		utils.HayekashDatasetsInMemoryFlag,
		utils.HayekashDatasetsOnDiskFlag,
		utils.HayekashDatasetsLockMmapFlag,
		...
		utils.MinerHayekerbaseFlag,
		utils.LegacyMinerHayekerbaseFlag,
...
func init() {
	// Initialize the CLI app and start Ghyk
	app.Action = ghyk
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2020 The go-hayekchain Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.LegacyTestnetFlag.Name):
		log.Info("Starting Ghyk on Ropsten testnet...")
	...
	case ctx.GlobalIsSet(utils.RinkebyFlag.Name):
		log.Info("Starting Ghyk on Rinkeby testnet...")

	case ctx.GlobalIsSet(utils.GoerliFlag.Name):
		log.Info("Starting Ghyk on Görli testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Ghyk in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Ghyk on HayekChain mainnet...")
	}
...
// ghyk is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func ghyk(ctx *cli.Context) error {
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, backend hykapi.Backend) {
	...
	// Create a client to interact with local ghyk node.
	rpcClient, err := stack.Attach()
	if err != nil {
		utils.Fatalf("Failed to attach to self: %v", err)
	}
	hykClient := hykclient.NewClient(rpcClient)
	...
	go func() {
		...
		for event := range events {
			...
			case accounts.WalletOpened:
				...
				event.Wallet.SelfDerive(derivationPaths, hykClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full HayekChain node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*hyk.HykAPIBackend)
		if !ok {
			utils.Fatalf("HayekChain service not running: %v", err)
		}
