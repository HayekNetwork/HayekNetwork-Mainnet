...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "ghyk-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "ghyk-test" in runGhyk.
	reexec.Register("ghyk-test", func() {
...
// spawns ghyk with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGhyk(t *testing.T, args ...string) *testghyk {
	tt := &testghyk{}
	...
	// Boot "ghyk". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("ghyk-test", args...)
