...
// Tests that initializing Ghyk with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	for i, tt := range customGenesisTests {
		...
		runGhyk(t, "--nousb", "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		ghyk := runGhyk(t, "--nousb", "--networkid", "1337", "--syncmode=full",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		ghyk.ExpectRegexp(tt.result)
		ghyk.ExpectExit()
	}
}
