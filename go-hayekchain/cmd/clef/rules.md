...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 hyker` in value to maximum `1 hyker` per 24h period
* I want to auto-approve transaction to contract `HykAlarmClock` with `data`=`0xdeadbeef`, if `value=0`, `gas < 44k` and `gasPrice < 40Gwei`
...
implemented for `ghyk`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
