...
Use `devp2p enrdump <base64>` to verify and display an HayekChain Node Record.
...
### Hyk Protocol Test Suite

The Hyk Protocol test suite is a conformance test suite for the [hyk protocol][hyk].

To run the hyk protocol test suite against your implementation, the node needs to be initialized as such:

1. initialize the ghyk node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run ghyk with the following flags:
```
ghyk --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode ID>` with the enode of the ghyk node: 
 ```
 devp2p rlpx hyk-test <enode ID> cmd/devp2p/internal/hyktest/testdata/fullchain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```
 
[hyk]: https://github.com/hayekchain/devp2p/blob/master/caps/hyk.md
[dns-tutorial]: https://ghyk.hayekchain.org/docs/developers/dns-discovery-setup
[discv4]: https://github.com/hayekchain/devp2p/tree/master/discv4.md
[discv5]: https://github.com/hayekchain/devp2p/tree/master/discv5/discv5.md
