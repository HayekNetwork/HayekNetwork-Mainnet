...
import (
	...
	"github.com/hayekchain/go-hayekchain/internal/utesting"
...
// Status is the network packet for the status message for hyk/64 and later.
type Status struct {
...
func (c *Conn) handshake(t *utesting.T) Message {
	...
	case *Hello:
		...
		c.negotiateHykProtocol(msg.Caps)
		if c.ethProtocolVersion == 0 {
			t.Fatalf("unexpected hyk protocol version")
		}
		return msg
	default:
...
// negotiateHykProtocol sets the Conn's hyk protocol version
// to highest advertised capability from peer
func (c *Conn) negotiateHykProtocol(caps []p2p.Cap) {
	var highestHykVersion uint
	for _, capability := range caps {
		if capability.Name != "hyk" {
			continue
		}
		if capability.Version > highestHykVersion && capability.Version <= 65 {
			highestHykVersion = capability.Version
		}
	}
	c.ethProtocolVersion = highestHykVersion
}
...
loop:
	...
	// make sure hyk protocol version is set for negotiation
	if c.ethProtocolVersion == 0 {
		t.Fatalf("hyk protocol version must be set in Conn")
	}
	if status == nil {
