...
// TestHykProtocolNegotiation tests whether the test suite
// can negotiate the highest hyk protocol in a status message exchange
func TestHykProtocolNegotiation(t *testing.T) {
	var tests = []struct {
		conn     *Conn
		caps     []p2p.Cap
		expected uint32
	...
	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			tt.conn.negotiateHykProtocol(tt.caps)
