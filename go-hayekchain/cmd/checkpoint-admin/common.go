...
import (
	...
	"github.com/hayekchain/go-hayekchain/contracts/checkpointoracle"
...
func newClient(ctx *cli.Context) *hykclient.Client {
	client, err := hykclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to HayekChain node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, hykclient.NewClient(client))
	if err != nil {
