...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"hykstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "hykstats":
				if infos, err := checkHykstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have hykstats running, ask whether to make the secret public or not
	if w.conf.hykstats != "" {
		fmt.Println()
		fmt.Println("Include hykstats secret on dashboard (y/n)? (default = yes)")
