...
FROM pupphyk/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'ghyk --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec ghyk --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --hykstats \'{{.Hykstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PupphykNetworkID/{{.NetworkID}}/g' dist/js/hykerwallet-master.js && \
	sed -i 's/PupphykNetwork/{{.Network}}/g'     dist/js/hykerwallet-master.js && \
	sed -i 's/PupphykDenom/{{.Denom}}/g'         dist/js/hykerwallet-master.js && \
	sed -i 's/PupphykHost/{{.Host}}/g'           dist/js/hykerwallet-master.js && \
	sed -i 's/PupphykRPCPort/{{.RPCPort}}/g'     dist/js/hykerwallet-master.js
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.hayekchain
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Hykstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
      - VIRTUAL_PORT=80{{end}}
    logging:
      driver: "json-file"
