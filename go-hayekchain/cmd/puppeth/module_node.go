...
// nodeDockerfile is the Dockerfile required to run an HayekChain node.
var nodeDockerfile = `
FROM hayekchain/client-go:latest
...
{{end}}
RUN \
  echo 'ghyk --cache 512 init /genesis.json' > ghyk.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.hayekchain/keystore/ && cp /signer.json /root/.hayekchain/keystore/' >> ghyk.sh && \{{end}}
	echo $'exec ghyk --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --hykstats \'{{.Hykstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Hayekerbase}}--miner.hykerbase {{.Hayekerbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> ghyk.sh

ENTRYPOINT ["/bin/sh", "ghyk.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an HayekChain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.hayekchain{{if .Hayekashdir}}
      - {{.Hayekashdir}}:/root/.hykash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Hykstats}}
      - MINER_NAME={{.Hayekerbase}}
...
// deployNode deploys a new HayekChain node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.hykerbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Hykstats":  config.hykstats,
		"Hayekerbase": config.hykerbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Hayekashdir":  config.hykashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Hykstats":   config.hykstats[:strings.Index(config.hykstats, ":")],
		"Hayekerbase":  config.hykerbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Hykstats username":        info.hykstats,
	}
	if info.gasTarget > 0 {
		...
		if info.hykerbase != "" {
			// Hayekash proof-of-work miner
			report["Hayekash directory"] = info.hykashdir
			report["Miner account"] = info.hykerbase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 ghyk --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.hayekchain"],
		hykashdir:  infos.volumes["/root/.hykash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		hykstats:   infos.envvars["STATS_NAME"],
		hykerbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
