...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Hayekers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.hykstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.hykstats = w.readString() + ":" + w.conf.hykstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.hykstats)
		infos.node.hykstats = w.readDefaultString(infos.node.hykstats) + ":" + w.conf.hykstats
	}
