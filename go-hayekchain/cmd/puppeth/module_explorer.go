...
FROM pupphyk/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'ghyk --cache 512 init /genesis.json' > explorer.sh && \
  echo $'ghyk --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.HykPort}} --bootnodes {{.Bootnodes}} --hykstats \'{{.Hykstats}}\' --cache=512 --http --http.api "net,web3,hyk,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec ghyk --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.HykPort}} --bootnodes {{.Bootnodes}} --hykstats \'{{.Hykstats}}\' --cache=512 --http --http.api "net,web3,hyk,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.HykPort}}:{{.HykPort}}"
            - "{{.HykPort}}:{{.HykPort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - HYK_PORT={{.HykPort}}
            - HYK_NAME={{.HykName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Hykstats":  config.node.hykstats,
		"HykPort":   config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"HykPort":     config.node.port,
		"HykName":     config.node.hykstats[:strings.Index(config.node.hykstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"HayekChain listener port ": strconv.Itoa(info.node.port),
		"Hykstats username":       info.node.hykstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["HYK_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.hayekchain"],
			port:     infos.portmap[infos.envvars["HYK_PORT"]+"/tcp"],
			hykstats: infos.envvars["HYK_NAME"],
		},
