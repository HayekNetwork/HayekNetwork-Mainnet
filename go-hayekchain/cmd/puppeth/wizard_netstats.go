...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for hykstats availability")
	if infos, err := checkHykstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["hykstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["hykstats"] = infos.Report()
		hykstats = infos.config
	}
