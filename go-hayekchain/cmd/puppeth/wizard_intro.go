...
// makeWizard creates and returns a new pupphyk wizard.
...
// setting up a new or managing an existing HayekChain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to pupphyk, your HayekChain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new HayekChain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and hykstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Pupphyk uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering HayekChain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".pupphyk", w.network)
