...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. Hayekash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of hykash, we're pretty much done
		genesis.Config.Hayekash = new(params.HayekashConfig)
...
// importGenesis imports a Ghyk genesis spec into pupphyk.
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alhyk.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by pupphyk and Ghyk
		ghykJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((ghykJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", ghykJson)

		// Export the genesis spec used by Aleth (formerly C++ HayekChain)
		...
		// Export the genesis spec used by Harmony (formerly HayekChainJ)
