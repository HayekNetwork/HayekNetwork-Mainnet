...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.hykstats == "" {
		log.Error("No hykstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Hayekash != nil && !boot {
		fmt.Println()
		if infos.hykashdir == "" {
			fmt.Printf("Where should the hykash mining DAGs be stored on the remote machine?\n")
			infos.hykashdir = w.readString()
		} else {
			fmt.Printf("Where should the hykash mining DAGs be stored on the remote machine? (default = %s)\n", infos.hykashdir)
			infos.hykashdir = w.readDefaultString(infos.hykashdir)
		}
	}
	...
	if infos.hykstats == "" {
		...
		infos.hykstats = w.readString() + ":" + w.conf.hykstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.hykstats)
		infos.hykstats = w.readDefaultString(infos.hykstats) + ":" + w.conf.hykstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Hayekash != nil {
			// Hayekash based miners only need an hykerbase to mine against
			fmt.Println()
			if infos.hykerbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.hykerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.hykerbase)
				infos.hykerbase = w.readDefaultAddress(common.HexToAddress(infos.hykerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy HayekChain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
