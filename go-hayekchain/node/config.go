...
import (
	...
	"github.com/hayekchain/go-hayekchain/accounts/external"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of ghyk is "ghyk". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Ghyk", keep that.
	if name == "ghyk" || name == "ghyk-testnet" {
		name = "Ghyk"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "ghyk" instances.
var isOldGhykResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by ghyk 1.4 are used if they exist.
	if warn, isOld := isOldGhykResource[path]; isOld {
		oldpath := ""
		if c.name() == "ghyk" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGhykResourceWarning, "Using deprecated resource file %s, please move this file to the 'ghyk' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-hayekchain-keystore")
		ephemeral = keydir
	}
