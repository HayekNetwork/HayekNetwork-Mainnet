...
func newTester(t *testing.T, confOverride func(*hyk.Config)) *tester {
	...
	// Create a networkless protocol stack and start an HayekChain service within
	...
	ethConf := &hyk.Config{
		...
		Miner: miner.Config{
			Hayekerbase: common.HexToAddress(testAddress),
		},
		Hayekash: hykash.Config{
			PowMode: hykash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(ethConf)
	}
	ethBackend, err := hyk.New(stack, ethConf)
	if err != nil {
		t.Fatalf("failed to register HayekChain protocol: %v", err)
	}
