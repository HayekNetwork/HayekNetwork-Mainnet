...
import (
	...
	"github.com/hayekchain/go-hayekchain/accounts/usbwallet"
	"github.com/hayekchain/go-hayekchain/common/hexutil"
	"github.com/hayekchain/go-hayekchain/console/prompt"
	"github.com/hayekchain/go-hayekchain/internal/jsre"
...
type bridge struct {
	client   *rpc.Client         // RPC client to execute HayekChain requests through
...
func getJeth(vm *goja.Runtime) *goja.Object {
	...
	return jhyk.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC method that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC method (saved in jhyk.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	if !callable {
		return nil, fmt.Errorf("jhyk.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	if !callable {
		return nil, fmt.Errorf("jhyk.openWallet is not callable")
	}
...
// original RPC method (saved in jhyk.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	if !callable {
		return nil, fmt.Errorf("jhyk.unlockAccount is not callable")
	}
...
// jhyk.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	if !callable {
		return nil, fmt.Errorf("jhyk.sign is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "hyk_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "hyk_blockNumber"); err != nil {
			return nil, err
		}
