...
import (
	...
	"github.com/hayekchain/go-hayekchain/internal/jsre/deps"
	"github.com/hayekchain/go-hayekchain/internal/web3ext"
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"hyk": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) initPersonal(vm *goja.Runtime, bridge *bridge) {
	...
	jhyk := vm.NewObject()
	vm.Set("jeth", jhyk)
	jhyk.Set("openWallet", personal.Get("openWallet"))
	jhyk.Set("unlockAccount", personal.Get("unlockAccount"))
	jhyk.Set("newAccount", personal.Get("newAccount"))
	jhyk.Set("sign", personal.Get("sign"))
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines hyk.getBalance(hyk.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Ghyk instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Ghyk JavaScript console!\n\n"

	// Print some generic Ghyk metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + hyk.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + hyk.blockNumber + " (" + new Date(1000 * hyk.getBlock(hyk.blockNumber).timestamp) + ")\n";
		try {
