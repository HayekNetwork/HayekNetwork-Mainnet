...
var (
	...
	RopstenChainConfig = &ChainConfig{
		...
		PetersburgBlock:     big.NewInt(49),
		IstanbulBlock:       big.NewInt(64),
		MuirGlacierBlock:    big.NewInt(71),
		Hayekash:              new(HayekashConfig),
	}
	...
	RinkebyChainConfig = &ChainConfig{
		...
		ConstantinopleBlock: big.NewInt(36),
		PetersburgBlock:     big.NewInt(43),
		IstanbulBlock:       big.NewInt(54),
	...
	// AllHayekashProtocolChanges contains every protocol change (EIPs) introduced
	// and accepted by the HayekChain core developers into the Hayekash consensus.
	//
	// This configuration is intentionally not using keyed fields to force anyone
	// adding flags to the config to also have to set these fields.
	AllHayekashProtocolChanges = &ChainConfig{big.NewInt(1337), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, nil, nil, new(HayekashConfig), nil}

	// AllCliqueProtocolChanges contains every protocol change (EIPs) introduced
	// and accepted by the HayekChain core developers into the Clique consensus.
	//
	// This configuration is intentionally not using keyed fields to force anyone
	// adding flags to the config to also have to set these fields.
	...
	TestChainConfig = &ChainConfig{big.NewInt(1), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, nil, nil, new(HayekashConfig), nil}
...
type ChainConfig struct {
	...
	DAOForkSupport bool     `json:"daoForkSupport,omitempty"` // Whhyker the nodes supports or opposes the DAO hard-fork

	// EIP150 implements the Gas price changes (https://github.com/hayekchain/EIPs/issues/150)
	...
	Hayekash *HayekashConfig `json:"hykash,omitempty"`
	Clique *CliqueConfig `json:"clique,omitempty"`
}

// HayekashConfig is the consensus engine configs for proof-of-work based sealing.
type HayekashConfig struct{}

// String implements the stringer interface, returning the consensus engine details.
func (c *HayekashConfig) String() string {
	return "hykash"
}
...
func (c *ChainConfig) String() string {
	...
	/*
	return fmt.Sprintf("{ChainID: %v Hayek: %v Engine: %v}",
		c.ChainID,
		c.MuirGlacierBlock,
		engine,
	)
...
func (c *ChainConfig) DecreaseRewardHeight(num *big.Int, epoch *big.Int) (*big.Int, *big.Int) {
	zero := big.NewInt(0)
	if num == nil || zero.Cmp(num) == 0 {
		return big.NewInt(0), big.NewInt(0)
	}
	r := big.NewInt(0)
	m := big.NewInt(0)
	r.DivMod(num, epoch, m)
	return r, m
}
...
// CheckConfigForkOrder checks that we don't "skip" any forks, ghyk isn't pluggable enough
