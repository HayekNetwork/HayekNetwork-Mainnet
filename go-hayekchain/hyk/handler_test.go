...
// Tests that post hyk protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	blockchain, err := core.NewBlockChain(db, nil, config, hykash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create new blockchain: %v", err)
	}
	pm, err := NewProtocolManager(config, cht, syncmode, DefaultConfig.NetworkId, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, hykash.NewFaker(), blockchain, db, 1, nil)
	if err != nil {
...
func testBroadcastBlock(t *testing.T, totalPeers, broadcastExpected int) {
	...
	chain, _ := core.GenerateChain(gspec.Config, genesis, hykash.NewFaker(), db, 1, func(i int, gen *core.BlockGen) {})
