...
import (
	...
	"github.com/hayekchain/go-hayekchain/common/prque"
...
type fetchRequest struct {
	...
	From    uint64          // [hyk/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [hyk/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead      common.Hash                    // [hyk/62] Hash of the last queued header to verify order
	headerTaskPool  map[uint64]*types.Header       // [hyk/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [hyk/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [hyk/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [hyk/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [hyk/62] Result cache accumulating the completed headers
	headerProced    int                            // [hyk/62] Number of headers already processed from the results
	headerOffset    uint64                         // [hyk/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [hyk/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[common.Hash]*types.Header // [hyk/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [hyk/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [hyk/62] Currently pending block (body) retrieval operations

	receiptTaskPool  map[common.Hash]*types.Header // [hyk/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [hyk/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [hyk/63] Currently pending receipt retrieval operations
