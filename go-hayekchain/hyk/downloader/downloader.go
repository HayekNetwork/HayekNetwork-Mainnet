...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [hyk/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    hykdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [hyk/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [hyk/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [hyk/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [hyk/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [hyk/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [hyk/62] Channel to feed the header processor new tasks
	...
	stateCh        chan dataPack // [hyk/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb hykdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() hayekchain.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "hyk", p.version, "head", hash, "td", td, "mode", mode)
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // Whhyker the next request is pivot verification
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put toghyker a new list of blocks to import, which is a superset
