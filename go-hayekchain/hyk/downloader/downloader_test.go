...
type downloadTester struct {
	...
	stateDb hykdb.Database // Database used by the tester for syncing from peers
	peerDb  hykdb.Database // Database of the peers containing all data
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", hayekchain.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", hayekchain.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", hayekchain.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", hayekchain.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want hayekchain.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", hayekchain.SyncProgress{
