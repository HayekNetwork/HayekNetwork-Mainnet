...
// stateReq represents a batch of state fetch requests grouped toghyker into
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < hykdb.IdealBatchSize {
		return nil
	}
	start := time.Now()
	...
	for _, p := range peers {
		...
		if len(nodes)+len(codes) > 0 {
			...
			case s.d.trackStateReq <- req:
				req.peer.FetchNodeData(append(nodes, codes...)) // Unified retrieval under hyk/6x
			case <-s.cancel:
			case <-s.d.cancelCh:
			}
		}
