...
const (
	// bloomServiceThreads is the number of goroutines used globally by an HayekChain
...
func (hyk *HayekChain) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-hyk.closeBloomHandler:
					return

				case request := <-hyk.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(hyk.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(hyk.chainDb, task.Bit, section, head); err == nil {
...
// for the HayekChain header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      hykdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db hykdb.Database, size, confirms uint64) *core.ChainIndexer {
