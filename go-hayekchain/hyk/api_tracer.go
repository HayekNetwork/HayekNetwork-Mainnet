...
import (
	...
	"github.com/hayekchain/go-hayekchain/hyk/tracers"
	_ "github.com/hayekchain/go-hayekchain/internal/hykapi"
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.hyk.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.hyk.blockchain.CurrentBlock()
	default:
		from = api.hyk.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.hyk.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.hyk.blockchain.CurrentBlock()
	default:
		to = api.hyk.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithConfig(api.hyk.ChainDb(), &trie.Config{Cache: 16, Preimages: true})

	if number := start.NumberU64(); number > 0 {
		start = api.hyk.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				signer := types.MakeSigner(api.hyk.blockchain.Config(), task.block.Number())
				blockCtx := core.NewEVMBlockContext(task.block.Header(), api.hyk.blockchain, nil)
				...
				for i, tx := range task.block.Transactions() {
					...
					task.statedb.Finalise(api.hyk.blockchain.Config().IsEIP158(task.block.Number()))
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.hyk.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.hyk.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
			// Finalize the state so any modifications are written to the trie
			root, err := statedb.Commit(api.hyk.blockchain.Config().IsEIP158(block.Number()))
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.hyk.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.hyk.blockchain.CurrentBlock()
	default:
		block = api.hyk.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.hyk.blockchain.GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) TraceBadBlock(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	blocks := api.hyk.blockchain.BadBlocks()
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.hyk.engine.VerifyHeader(api.hyk.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.hyk.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		signer = types.MakeSigner(api.hyk.blockchain.Config(), block.Number())
	...
	blockCtx := core.NewEVMBlockContext(block.Header(), api.hyk.blockchain, nil)
	for th := 0; th < threads; th++ {
		pend.Add(1)
		go func() {
			defer pend.Done()
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(blockCtx, txContext, statedb, api.hyk.blockchain.Config(), vm.Config{})
...
func (api *PrivateDebugAPI) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	var (
		signer      = types.MakeSigner(api.hyk.blockchain.Config(), block.Number())
		dumps       []string
		chainConfig = api.hyk.blockchain.Config()
		vmctx       = core.NewEVMBlockContext(block.Header(), api.hyk.blockchain, nil)
		canon       = true
	)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	statedb, err := api.hyk.blockchain.StateAt(block.Root())
	if err == nil {
		return statedb, nil
	}
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.hyk.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.hyk.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := rawdb.ReadTransaction(api.hyk.ChainDb(), hash)
	if tx == nil {
	...
	// Retrieve the block
	block := api.hyk.blockchain.GetBlockByHash(blockHash)
	if block == nil {
...
// TraceCall lets you trace a given hyk_call. It collects the structured logs created during the execution of EVM
// if the given transaction was added on top of the provided block and returns them as a JSON object.
// You can provide -2 as a block number to trace on top of the pending block.
func (api *PrivateDebugAPI) TraceCall(ctx context.Context, args hykapi.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceConfig) (interface{}, error) {
	// First try to retrieve the state
	statedb, header, err := api.hyk.APIBackend.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	// Execute the trace
	msg := args.ToMessage(api.hyk.APIBackend.RPCGasCap())
	vmctx := core.NewEVMBlockContext(header, api.hyk.blockchain, nil)
	return api.traceTx(ctx, msg, vmctx, statedb, config)
}
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, txContext, statedb, api.hyk.blockchain.Config(), vm.Config{Debug: true, Tracer: tracer})
	...
	case *vm.StructLogger:
		...
		return &hykapi.ExecutionResult{
			Gas:         result.UsedGas,
			Failed:      result.Failed(),
			ReturnValue: returnVal,
			StructLogs:  hykapi.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		txContext := core.NewEVMTxContext(msg)
		context := core.NewEVMBlockContext(block.Header(), api.hyk.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, txContext, statedb, api.hyk.blockchain.Config(), vm.Config{})
