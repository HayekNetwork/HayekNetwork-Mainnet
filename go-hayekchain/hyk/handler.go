...
import (
	...
	"github.com/hayekchain/go-hayekchain/hyk/fetcher"
...
// NewProtocolManager returns a new HayekChain sub protocol manager. The HayekChain sub protocol manages peers capable
// with the HayekChain network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb hykdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing HayekChain peer", "peer", id)

	// Unregister the peer from the downloader and HayekChain peer set
...
func (pm *ProtocolManager) Start(maxPeers int) {
	...
	go pm.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with hyk/64.
}
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("HayekChain protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an hyk peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("HayekChain peer connected", "name", p.Name())

	// Execute the HayekChain handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkID, pm.forkFilter); err != nil {
		p.Log().Debug("HayekChain handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p, pm.removePeer); err != nil {
		p.Log().Error("HayekChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("HayekChain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == NewPooledTransactionHashesMsg && p.version >= hyk65:
	...
	case msg.Code == GetPooledTransactionsMsg && p.version >= hyk65:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case msg.Code == TransactionMsg || (msg.Code == PooledTransactionsMsg && p.version >= hyk65):
...
// NodeInfo represents a short summary of the HayekChain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // HayekChain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
