...
// Package filters implements an hayekchain filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  hayekchain.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit hayekchain.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit hayekchain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit hayekchain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit hayekchain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
