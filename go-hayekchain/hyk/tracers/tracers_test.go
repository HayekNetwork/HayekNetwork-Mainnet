...
import (
	...
	"github.com/hayekchain/go-hayekchain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Ghyk console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = hyk.getBlock(hyk.getTransaction(tx).blockHash);
  var genesis = hyk.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.hyk.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  hyk.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
