...
import (
	...
	"github.com/hayekchain/go-hayekchain/crypto/signify"
	"github.com/hayekchain/go-hayekchain/internal/build"
	"github.com/hayekchain/go-hayekchain/params"
)

var (
	// Files that end up in the ghyk*.zip archive.
	ghykArchiveFiles = []string{
		"COPYING",
		executablePath("ghyk"),
	}

	// Files that end up in the ghyk-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert HayekChain contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "HayekChain bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (HayekChain Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "ghyk",
			Description: "HayekChain CLI client.",
		},
		{
			BinaryName:  "pupphyk",
			Description: "HayekChain private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "HayekChain account management tool.",
		},
	}

	// A debian package is created for all executables listed here.

	debHayekChain = debPackage{
		Name:        "hayekchain",
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 13 {
			...
			log.Println("go-hayekchain requires at least Go version 1.13 and cannot")
...
func doTest(cmdline []string) {
	coverage := flag.Bool("coverage", false, "Whhyker to record code coverage")
	verbose := flag.Bool("v", false, "Whhyker to log verbosely")
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "ghykstore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		baseghyk = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		ghyk     = "ghyk-" + baseghyk + ext
		alltools = "ghyk-alltools-" + baseghyk + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(ghyk, ghykArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{ghyk, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with ghyk-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "hayekchain/hayekchain")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "ghyk-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-hayekchain sources.
...
func downloadGo(goarch, goos, cachedir string) string {
	...
	godir := filepath.Join(ucache, fmt.Sprintf("ghyk-go-%s-%s-%s", dlgoVersion, goos, goarch))
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "ghyk-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "hayekchain"
...
type debMetadata struct {
	...
	// go-hayekchain version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "HayekChain Builds <fjl@hayekchain.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "ghyk.exe" {
			ghykTool = file
		} else {
	...
	// first section contains the ghyk binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Ghyk":     ghykTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.ghyk.nsi", filepath.Join(*workdir, "ghyk.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("ghyk-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "ghyk.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "ghykstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.hayekchain", "-v", "github.com/hayekchain/go-hayekchain/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("ghyk.aar", filepath.Join(GOBIN, "ghyk.aar"))
		os.Rename("ghyk-sources.jar", filepath.Join(GOBIN, "ghyk-sources.jar"))
		return
	}
	...
	archive := "ghyk-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("ghyk.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/hayekchain/go-hayekchain/mobile")
	...
	archive := "ghyk-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Ghyk.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Ghyk.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "ghykstore/builds")`)
