...
import (
	...
	"github.com/hayekchain/go-hayekchain/accounts/usbwallet/trezor"
...
// or whether the HayekChain app was not started on it.
...
// HayekChain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.HayekChainAddress)
	if _, err := w.trezorExchange(&trezor.HayekChainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.HayekChainSignTx{
	...
	response := new(trezor.HayekChainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.HayekChainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
