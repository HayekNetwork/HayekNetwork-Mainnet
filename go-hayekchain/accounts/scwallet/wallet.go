...
type Wallet struct {
	...
	deriveChain     hayekchain.ChainStateReader // Blockchain state reader to discover used account with
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived HayekChain account
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain hayekchain.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
type Session struct {
	...
	verified bool                  // Whhyker the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whhyker the card has been initialized with a private key
}
