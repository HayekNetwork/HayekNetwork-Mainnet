...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != hayekchain.NotFound {
		t.Fatalf("err should be `hayekchain.NotFound` but received %v", err)
	}
...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != params.AllHayekashProtocolChanges {
		t.Errorf("expected sim config to equal params.AllHayekashProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != params.AllHayekashProtocolChanges {
		t.Errorf("expected sim blockchain config to equal params.AllHayekashProtocolChanges, got %v", sim.config)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.Hayeker)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		name        string
		message     hayekchain.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", hayekchain.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", hayekchain.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.Hayeker*2 + 2e17)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", hayekchain.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", hayekchain.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", hayekchain.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(1e14), // gascost = 2.1hyker
			Value:    big.NewInt(1e17), // the remaining balance for fee is 2.1hyker
			Data:     nil,
		}, 21000, nil},

		{"EstimateWithSuperhighPrice", hayekchain.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(2e14), // gascost = 4.2hyker
			Value:    big.NewInt(1000),
			Data:     nil,
		}, 21000, errors.New("gas required exceeds allowance (10999)")}, // 10999=(2.2hyker-1000wei)/(2e14)
	}
	for _, c := range cases {
		got, err := sim.EstimateGas(context.Background(), c.message)
		if c.expectError != nil {
...
func TestSimulatedBackend_PendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, hayekchain.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, hayekchain.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestSimulatedBackend_CallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, hayekchain.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, hayekchain.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
