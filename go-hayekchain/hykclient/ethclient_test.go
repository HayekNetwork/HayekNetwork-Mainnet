...
// Verify that Client implements the hayekchain interfaces.
var (
	_ = hayekchain.ChainReader(&Client{})
	_ = hayekchain.TransactionReader(&Client{})
	_ = hayekchain.ChainStateReader(&Client{})
	_ = hayekchain.ChainSyncReader(&Client{})
	_ = hayekchain.ContractCaller(&Client{})
	_ = hayekchain.GasEstimator(&Client{})
	_ = hayekchain.GasPricer(&Client{})
	_ = hayekchain.LogFilterer(&Client{})
	_ = hayekchain.PendingStateReader(&Client{})
	// _ = hayekchain.PendingStateEventer(&Client{})
	_ = hayekchain.PendingContractCaller(&Client{})
)
...
func TestToFilterArg(t *testing.T) {
	...
	for _, testCase := range []struct {
		name   string
		input  hayekchain.FilterQuery
		output interface{}
		err    error
	}{
		{
			"without BlockHash",
			hayekchain.FilterQuery{
				Addresses: addresses,
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create HayekChain Service
	config := &hyk.Config{Genesis: genesis}
	config.Hayekash.PowMode = hykash.ModeFake
	ethservice, err := hyk.New(n, config)
	if err != nil {
		t.Fatalf("can't create new hayekchain service: %v", err)
	}
...
func TestChainID(t *testing.T) {
	...
	if id == nil || id.Cmp(params.AllHayekashProtocolChanges.ChainID) != 0 {
