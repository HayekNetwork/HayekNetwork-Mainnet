...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useHayekChainV = true // Default to use V = 27 or 28, the legacy HayekChain format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useHayekChainV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the HayekChain PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useHayekChainV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
		...
		useHayekChainV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: cliqueRlp, Messages: messages, Hash: sighash}
	default: // also case TextPlain.Mime:
		// Calculates an HayekChain ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}HayekChain Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useHayekChainV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
