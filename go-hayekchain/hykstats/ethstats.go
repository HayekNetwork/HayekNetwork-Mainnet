...
// Package hykstats implements the network stats reporting service.
package hykstats
...
// backend encompasses the bare-minimum functionality needed for hykstats reporting
...
// Service implements an HayekChain netstats reporting daemon that pushes local
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(hykstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["hyk"]; info != nil {
		network = fmt.Sprintf("%d", info.(*hyk.NodeInfo).Network)
		protocol = fmt.Sprintf("hyk/%d", hyk.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the hykstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to hykstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to hykstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to hykstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to hykstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to hykstats")
