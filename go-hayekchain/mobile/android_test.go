...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Ghyk.LightScryptN, Ghyk.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.hayekchain", new NodeConfig());
			node.start();
			...
			HayekChainClient ec = node.getHayekChainClient();
	...
	// signatures too. Regression test for go-hayekchain issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoghyker
	...
	workspace, err := ioutil.TempDir("", "ghyk-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/hayekchain/ghyktest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Ghyk and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.hayekchain", "github.com/hayekchain/go-hayekchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "ghyk.aar"), "ghyk.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "hayekchain", "ghyktest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.hayekchain.ghyktest"
