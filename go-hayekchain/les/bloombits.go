...
import (
	_ "github.com/hayekchain/go-hayekchain/hyk"
	"time"
...
func (hyk *LightHayekChain) startBloomHandlers(sectionSize uint64) {
		...
		go func() {
			...
			for {
				...
				case request := <-hyk.bloomRequests:
					...
					compVectors, err := light.GetBloomBits(task.Context, hyk.odr, task.Bit, task.Sections)
					if err == nil {
