...
import (
	...
	lps "github.com/hayekchain/go-hayekchain/les/lespay/server"
...
const (
	...
	hykVersion        = 63              // equivalent hyk version for the downloader
...
func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb hykdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light HayekChain peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), forkID, h.forkFilter, h.server); err != nil {
		p.Log().Debug("Light HayekChain handshake failed", "err", err)
		return err
	}
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light HayekChain peer rejected", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light hayekchain response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light HayekChain message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light HayekChain message arrived", "code", msg.Code, "bytes", msg.Size)
