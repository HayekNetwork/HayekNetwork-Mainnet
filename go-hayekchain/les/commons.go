...
func (c *lesCommons) setupOracle(node *node.Node, genesis common.Hash, hykconfig *hyk.Config) *checkpointoracle.CheckpointOracle {
	config := hykconfig.CheckpointOracle
	if config == nil {
	...
	client := hykclient.NewClient(rpcClient)
	oracle.Start(client)
