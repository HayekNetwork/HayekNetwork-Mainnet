...
import (
	...
	"github.com/hayekchain/go-hayekchain/p2p/simulations"
	"github.com/hayekchain/go-hayekchain/p2p/simulations/adapters"
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "hyk_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "hyk_getBalance", addr, "latest")
	if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	...
	hayekchain, err := hyk.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLesServer(stack, hayekchain, &config)
	if err != nil {
		return nil, err
	}
	return hayekchain, nil
}
