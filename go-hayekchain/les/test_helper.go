...
import (
	...
	"github.com/hayekchain/go-hayekchain/accounts/abi/bind/backends"
	...
	"github.com/hayekchain/go-hayekchain/contracts/checkpointoracle/contract"
	...
	"github.com/hayekchain/go-hayekchain/les/checkpointoracle"
	"github.com/hayekchain/go-hayekchain/les/flowcontrol"
	...
	"github.com/hayekchain/go-hayekchain/p2p/nodestate"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some hyker to user1
		...
		case 1:
			...
			// bankUser transfers more hyker to user1
		...
		case 2:
			// bankUser transfer some hyker to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db hykdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
	var indexers [3]*core.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms, disablePruning)
	indexers[1] = hyk.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db hykdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
		evmux  = new(event.TypeMux)
		engine = hykash.NewFaker()
		gspec  = core.Genesis{
			Config:   params.AllHayekashProtocolChanges,
	...
	client := &LightHayekChain{
		lesCommons: lesCommons{
			genesis:     genesis.Hash(),
			config:      &hyk.Config{LightPeers: 100, NetworkId: NetworkId},
			chainConfig: params.AllHayekashProtocolChanges,
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db hykdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllHayekashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
